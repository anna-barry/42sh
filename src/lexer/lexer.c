    ...
    *i = 0;
    ...
    for (; *i <= strlen(input); *i = *i + 1)
    {
        ...
        if (*info->cap < (*info->index + 2))
        {
            *info->cap *= 2;
            info->lexer =
                realloc(info->lexer, *info->cap * sizeof(struct lexer));
        }
        switch (input[*i])
        {
        case ('\n'):
        ...
        case ('<'):
            ...
            {
                info->lexer[*info->index - 1].current_tok =
                    token_new(TOKEN_REDIR_INPUT_DESCRIPEUR);
                nb = 2;
                while (input[*i + nb] == ' ')
                {
                    nb++;
                ...
                if (input[*i + 2] >= '0' && input[*i + 2] <= '9')
                {
                    ...
                    info->lexer[*info->index - 1].current_tok->value =
                        strndup(input + *i + tmp, nb);
                    info->lexer[*info->index - 1].current_tok->value[nb - tmp] =
                        '\0';
                    nb--;
                }
                else
        ...
        case ('>'):
            ...
            {
                info->lexer[*info->index - 1].current_tok =
                    token_new(TOKEN_REDIR_DESCRIPEUR);
                nb = 2;
                while (input[*i + nb] == ' ')
                {
                    nb++;
            ...
            {
                info->lexer[*info->index - 1].current_tok =
                    token_new(TOKEN_REDIR_FIN_FICHIER);
                nb = 2;
                while (input[*i + nb] == ' ')
                {
                    nb++;
        ...
        case ('\''):
            ...
            info->lexer[*info->index - 1].current_tok->value =
                strndup(input + *i + 1, nb - 1);
            info->lexer[*info->index - 1].current_tok->value[nb - 1] = '\0';
            break;
        case ('\"'):
            info->lexer[*info->index - 1].current_tok =
                token_new(TOKEN_DOUBLE_QUOTE);
            nb = 1;
            while (*i + nb < strlen(input) && input[*i + nb] != '\0'
                   && input[*i + nb] != '\"')
            {
...
struct token *lexer_pop(struct lexer *lexer)
{
    ...
    char *test = NULL;
    if (t == TOKEN_SIMPLE_QUOTE || t == TOKEN_WORDS || t == TOKEN_REDIR_SORTIE
        || t == TOKEN_REDIR_ENTREE || t == TOKEN_REDIR_DESCRIPEUR
        || t == TOKEN_REDIR_FIN_FICHIER || t == TOKEN_DOUBLE_QUOTE
        || t == TOKEN_FOR_WORD || t == TOKEN_REDIR_PIPE
        || t == TOKEN_FOR_SINGLE_QUOTE || t == TOKEN_FOR_DOUBLE_QUOTE
    ...
    while (lexer[j].current_tok->type != TOKEN_EOF)
    {
            ...
            || lexer[j + 1].current_tok->type == TOKEN_REDIR_PIPE)
        {
            ...
            lexer[j].current_tok->value =
                strndup(lexer[j + 1].current_tok->value,
    ...
    if (t == TOKEN_SIMPLE_QUOTE || t == TOKEN_WORDS || t == TOKEN_REDIR_SORTIE
        ...
        || t == TOKEN_REDIR_FIN_FICHIER || t == TOKEN_DOUBLE_QUOTE
        || t == TOKEN_FOR_WORD || t == TOKEN_REDIR_PIPE
        || t == TOKEN_REDIR_INPUT_DESCRIPEUR || t == TOKEN_REDIR_RW
        || t == TOKEN_FOR_SINGLE_QUOTE || t == TOKEN_FOR_DOUBLE_QUOTE
        || t == TOKEN_FOR_INT)
    {
